function mainTestTimeStepper(theta, Re, C, xL, yL, tFinal, xN, yN, timeStepper, timeStep, tStepOut, RelTol, timeout)
    if nargin < 12
        timeout = -1;
    else
    ...
    tWindup = reshape( ...
        (0:sscanf(func2str(timeStepper),"bdf%dsi")-1)*timeStep, ...
        1, 1, []);
    h0 = 1 - params.a * cos(2*pi*domain.x{1}/xL - params.c * tWindup) ...
        - params.b * cos(2*pi*domain.x{2}/yL);

    y0 = squeeze(domain.reshapeToVector(h0));
    ...
    myoptimoptions = optimoptions('fsolve', ...
        'Display', 'off', ...
        'StepTolerance', RelTol * sqrt(xN * yN), ...
