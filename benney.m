function [F,J] = benney(x,y,params,method,getD)
    ...
    if nargout > 1
        ...
        v = reshape(y,numel(y),1);
        dv = cellfun(@(z) reshape(z,numel(y),1),dy,...
        'UniformOutput',false);
        
        q1_h = diag(2*v.^2.*(1 - dv{1}*cot(theta) + ...
            (Lap_Dx * v)/(2*C))) + ...
            diag(2*v.^3/3)*(-cot(theta)*D{1} + Lap_Dx/(2*C));
        q1_h = q1_h + ...
            diag(Re*8*v.^6/15)*D{1} + ...
            diag(Re*16*v.^5.*dv{1}/5);
        
        q2_h = diag(2*v.^2.*(- dv{2}*cot(theta) + ...
            (Lap_Dy * v)/(2*C))) + ...
            diag(2*v.^3/3)*(-cot(theta)*D{2} + Lap_Dy/(2*C));
